...
اینم تصویر ساختار روت بندی داینامیک
<img src="./readmeFiled/API.PNG" />
🚀 بسته به نیاز پروژه، یکی از روش‌های بالا را انتخاب کنید.



### **📌 فایل README برای مدیریت متادیتا در Next.js**  
> **راهنمای تنظیم متادیتای داینامیک و استاتیک در Next.js 13+**  

---

## **📖 مقدمه**  
در Next.js 13+، می‌توان **متادیتای سئو (SEO)** را به‌صورت **استاتیک** یا **داینامیک** مدیریت کرد. این متادیتا می‌تواند شامل **عنوان (`title`)، توضیحات (`description`)، و OpenGraph metadata** برای **اشتراک‌گذاری در شبکه‌های اجتماعی** باشد.  

---

## **📌 تنظیم متادیتای استاتیک برای کل سایت**  
برای تنظیم متادیتای **پیش‌فرض** در سطح **کل پروژه**، باید آن را در **`app/layout.tsx`** یا **`app/layout.js`** قرار دهید.  
...
export const metadata: Metadata = {
  title: {
    default: "وب‌سایت من",
    template: "%s | وب‌سایت من", // قالب پیش‌فرض عنوان صفحات
  },
...
```
✅ **نتیجه:**  
- تمام صفحات به‌صورت پیش‌فرض دارای **`وب‌سایت من`** در عنوان خواهند بود.  
- متادیتای OpenGraph برای **اشتراک‌گذاری در شبکه‌های اجتماعی** تنظیم شده است.  
- قالب **"%s | وب‌سایت من"** به این معنی است که **هر صفحه** مقدار خودش را در `generateMetadata` ارسال می‌کند.  

---

## **📌 تنظیم متادیتای داینامیک برای هر صفحه**  
اگر بخواهید **عنوان و توضیحات هر صفحه را داینامیک کنید**، می‌توانید از **`generateMetadata`** استفاده کنید.  

📂 **مسیر:** `app/[slug]/page.tsx`  
```typescript
import type { Metadata } from "next";
...
export async function generateMetadata({ params }): Promise<Metadata> {
  ...
  // دریافت اطلاعات صفحه از API
  const response = await fetch(`https://api.example.com/meta/${slug}`);
  const data = await response.json();

  return {
    title: data.title || "صفحه پیش‌فرض",
    description: data.description || "توضیحی برای این صفحه موجود نیست.",
    openGraph: {
      title: data.title,
...
export default function Page({ params }) {
  return <h1>صفحه {params.slug}</h1>;
}
```
✅ **نتیجه:**  
- عنوان و توضیحات هر صفحه **بر اساس دیتابیس یا API** مقداردهی می‌شود.  
- در **OpenGraph metadata** مقدار داینامیک برای **SEO و شبکه‌های اجتماعی** تنظیم شده است.  

---

## **📌 تفاوت متادیتای داینامیک و استاتیک**
| نوع متادیتا | کد استفاده‌شده | کاربرد |
|------------|--------------|---------|
| **استاتیک** | `export const metadata` در `layout.tsx` | برای تنظیم **متادیتای پیش‌فرض** برای کل سایت |
| **داینامیک** | `generateMetadata({ params })` در `page.tsx` | برای تنظیم متادیتا **بر اساس داده‌های API یا دیتابیس** در هر صفحه |

---

## **📌 جمع‌بندی**
🔹 **اگر متادیتای یکسان برای کل سایت نیاز دارید** → از **متادیتای استاتیک (`metadata`) در `layout.tsx`** استفاده کنید.  
🔹 **اگر متادیتا باید بر اساس داده‌های هر صفحه تغییر کند** → از **`generateMetadata` در `page.tsx`** استفاده کنید.  

🚀 **Next.js 13+ بهینه‌ترین روش را برای مدیریت متادیتای داینامیک و استاتیک فراهم کرده است!**